...
double size = 80 ; 
    if (argc != 2) {
        std::cerr << "Usage: " << argv[0] << " <iteration>" << std::endl;
        return 1;
    }
size = std::stoi(argv[1]);

ofstream final_result("final_result_graph.txt") ; 
...
vector<double>train_accuracy ;
   for(int iter = 1 ; iter<=20 ; iter++)
   {
    ...
    cout<<size<<" "<<SD(test_accuracy)<<endl ; 
    //cout<<" Standard deviation of accuracy : "<<SD(test_accuracy)<<endl ; 
